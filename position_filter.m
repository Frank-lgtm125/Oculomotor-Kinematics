%% remove data with saccades, blinking, and off primary positions and  offset
...
function [filtered, result]= position_filter(data, params) 
    ...
    tol = params.threshs(1); %position threshold for initial ehp/evp left/right position
    pre_thresh = params.threshs(2); % eye velocity threshold of primate prior to stimulus
    dur_thresh = params.threshs(3); % eye velocity threshold of primates during stimulus to filter blinking. 
    pre_thresh_rest = params.threshs(4); % vertical eye velocity threshold prior to stimulus
    dur_thresh_rest = params.threshs(5); % vertical minor movement during to stimulus

    ehp_left_3d = data.ehp_left;
    ehp_right_3d = data.ehp_right;
    evp_left_3d = data.evp_left;
    evp_right_3d = data.evp_right;
    %% check if there is ehv_left stored in data
  ...
  %% find the reponse satisfing both ehp and evp inital position threshold
