...
for i=1:length(Refined_Data2)
   ...
   % plot all ipsilateral eye horizontal position from one processed BR file
    ...
    plot(temp.timeframe, temp.ehp_ipsi,'DisplayName', sprintf('%0.2d uA %i ms %i Hz',temp.current, temp.dur, temp.freq));
   ...
   % velocity 
    subplot(2,2,4)
    plot(temp.timeframe, 1000*temp.ipsi_ehv_avg, 'DisplayName', sprintf('%0.2d uA %i ms %i Hz',temp.current, temp.dur, temp.freq));
