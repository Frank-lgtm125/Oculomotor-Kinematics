...
function [filtered, result]= velocity_filter(data, params) 
    ...
    pre_thresh = params.threshs(2); % horizontal eye velocity threshold for alpha
    dur_thresh = params.threshs(3); % horizontal eye velocity threshold for during
%    post_thresh = params.threshs(4); % horizontal eye velocity threshold for omega
    pre_thresh_rest = params.threshs(4); % vertical eye velocity threshold for alpha
    dur_thresh_rest = params.threshs(5); % vertical eye velocity threshold for during
%    post_thresh_rest = params.threshs(7); %vertical eye velocity threshold for omega
...
% initialize the data
    ...
    if isempty(filter1) ~= 1 || isempty(filter2) ~=1
