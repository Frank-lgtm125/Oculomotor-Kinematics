%% This function is to calculate the confidence interval of the average ipsi/contra ehp/ehv/evp/evv
function output = post_process(processed_data)
alpha = 0.05; % alpha: significance level (e.g., 0.05 for 95% confidence interval)
output =cell(size(processed_data,1), 1); % create variable for the function output
for i = 1: size(processed_data, 1) % assign value from processed_data to temp
    temp = processed_data{i}; 
    num_bootstrap = size(temp.timeframe, 2); % get the sample size
    ...
    %% calculate ipsi/contra ehp/ehv for each remained BR file
    ...
    %% calculate ipsi/contra evp/evv for each remained BR file
...
%% This is the function to calculate the t-confidence interval 
